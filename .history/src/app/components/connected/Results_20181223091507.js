...
const showError = () => (
  <h3 className="Results-Error Results-ErrorTechnical">{counterpart('results.technicalError')}gdfgdgd</h3>
)
...
class ResultsContainer extends Component {
  ...
  constructor(props) {
    ...
    this._closeDetails = this
      ._closeDetails
      .bind(this)
    this._fetchMoreResults = this
      ._fetchMoreResults
      .bind(this)
    this._showProductDetails = this
      ._showProductDetails
      .bind(this)
    this._showNextProductDetails = this
      ._showNextProductDetails
      .bind(this)
    this._showPrevProductDetails = this
  ...
  componentDidMount() {
    const selectedCategory = this.props.hardcodedCategories
      ...
      : getAppParam('c')
        ? buildCategoryObjectFromName(getAppParam('c'))
        : null
    this
      .props
    ...
    this
      .props
      .fetchResults(getAppParam('q'), selectedCategory, this.props.age)
  }
  ...
  _fetchMoreResults() {
    ...
    this
      .props
      .fetchResults(this.props.term, this.props.selectedCategory, this.props.age, results.length)
  }

  _showProductDetails(id) {
    debugger
    this.setState({modalIsOpen: true, resultIdForDetails: null})
    this
      .props
      .selectResult(id)
    trackModalOpen(id)
  ...
  render() {
    ...
    if (displayedResults) {
      ...
      const trimmedResults = maxItems
        ? _take(displayedResults, maxItems)
        : displayedResults
      ...
      return (
        <Flexbox flexWrap="wrap" className="ResultsContainer" maxWidth="100%">
          ...
          {/* modal */}
          {(this.state.modalIsOpen) && (<DetailsModal
    ...
    return (
      <Flexbox flexWrap="wrap" className="ResultsContainer" maxWidth="100%"></Flexbox>
    )
  }
}
...
const Results = connect(mapStateToProps, mapDispatchToProps)(ResultsContainer)
