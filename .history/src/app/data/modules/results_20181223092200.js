...
export const resetResults = (categoryKey) => {
  return {type: RESET_RESULTS, categoryKey}
}

export const requestResults = (term, categories) => {
  return {type: FETCH_RESULTS, term, categories}
}
...
export const failedfetchingResults = (term, category) => {
  return {type: FETCH_RESULTS_FAILURE, term, category}
}

export const selectResult = id => {
  return {type: SELECT_RESULT, id}
}

export const resetSelectedResult = () => {
  return {type: RESET_SELECTED_RESULT}
}
...
export const resultsReducer = (state = initialState, action) => {
  switch (action.type) {
    case RESET_RESULTS:
      const storeCategory = action.categoryKey
        ? action.categoryKey
        : 'term'
    ...
    case FETCH_RESULTS_SUCCESS:
      const categoryName = action.selectedCategory
        ? action.selectedCategory.name
        : 'term'
      ...
      case FETCH_RESULTS_FAILURE : return Object.assign({}, state, {
        isFetching: false,
        hasFailedFetching: true
      })
    case SELECT_RESULT:
      return Object.assign({}, state, {selectedResult: action.id})
    case RESET_SELECTED_RESULT:
      return Object.assign({}, state, {selectedResult: null})
    default:
      return state
  }
}
...
export const fetchResults = (term, category, ages, offset = 0) => dispatch => {
  ...
  const url = buildUrl(`${process.env.REACT_APP_API_HOST}${process.env.REACT_APP_API_RESULTS_ENDPOINT}`, queryParams)
  return fetch(url, {
      method: 'GET',
      headers: {
