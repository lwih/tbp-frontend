...
const resultsRequest = (searchParams, offset) => {
  ...
  } else if (searchParams.cc) {
    params.cc = _get(searchParams, 'cc')
...
class Results extends React.Component {
  ...
  componentDidMount() {
    ...
    const url = this.props.searchParams
      ? resultsRequest(this.props.searchParams, this.state.offset)
      : `https://api.thebetterplay.com/product/search?&image_sizes=medium&age_until=1200&age_from=0`
  ...
  render() {
    ...
    if (
      (this.state.isFetching && this.state.offset === 0) ||
      !this.state.hasStartedFetchingOnce
    ) {
      return (
        <Flex flexWrap="wrap">
